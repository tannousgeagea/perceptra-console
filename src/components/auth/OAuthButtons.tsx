// components/OAuthButtons.tsx
import { useState } from 'react';
import { Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/ui/button';
import { useAuth } from '@/contexts/AuthContext';
import { OAuthProviderType } from '@/types/auth';

interface OAuthButtonsProps {
  className?: string;
}

const MicrosoftIcon = () => (
  <svg className="w-5 h-5" viewBox="0 0 23 23" fill="none">
    <path d="M0 0h11v11H0z" fill="#F25022"/>
    <path d="M12 0h11v11H12z" fill="#7FBA00"/>
    <path d="M0 12h11v11H0z" fill="#00A4EF"/>
    <path d="M12 12h11v11H12z" fill="#FFB900"/>
  </svg>
);

const GoogleIcon = () => (
  <svg className="w-5 h-5" viewBox="0 0 24 24">
    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
  </svg>
);

export const OAuthButtons: React.FC<OAuthButtonsProps> = ({ className = '' }) => {
  const { loginWithOAuth } = useAuth();
  const [loading, setLoading] = useState<OAuthProviderType | null>(null);

  const handleOAuthLogin = async (provider: OAuthProviderType) => {
    try {
      setLoading(provider);
      await loginWithOAuth(provider);
      // User will be redirected, no need to handle response
    } catch (error) {
      console.error(`${provider} login failed:`, error);
      setLoading(null);
      // Show error toast/notification here
    }
  };

  return (
    <div className={`space-y-3 ${className}`}>
      {/* Microsoft Button */}
      <Button
        type="button"
        variant="outline"
        size="lg"
        onClick={() => handleOAuthLogin('microsoft')}
        disabled={loading !== null}
        className="w-full relative group overflow-hidden transition-all duration-300 hover:border-primary/50 hover:shadow-md"
      >
        <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-accent/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        {loading === 'microsoft' ? (
          <Loader2 className="h-5 w-5 animate-spin" />
        ) : (
          <>
            <MicrosoftIcon />
            <span className="ml-3 font-medium">Continue with Microsoft</span>
          </>
        )}
      </Button>

      {/* Google Button (disabled until implemented) */}
      <Button
        type="button"
        variant="outline"
        size="lg"
        onClick={() => handleOAuthLogin('google')}
        disabled={loading !== null}
        className="w-full relative group overflow-hidden transition-all duration-300 hover:border-primary/50 hover:shadow-md"
      >
        <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-accent/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        {loading === 'google' ? (
          <Loader2 className="h-5 w-5 animate-spin" />
        ) : (
          <>
            <GoogleIcon />
            <span className="ml-3 font-medium">
              Continue with Google
            </span>
          </>
        )}
      </Button>

      {/* Divider */}
      <div className="relative">
        <div className="absolute inset-0 flex items-center">
          <div className="w-full border-t border-gray-300"></div>
        </div>
        <div className="relative flex justify-center text-sm">
          <span className="px-2 bg-white text-gray-500">Or continue with email</span>
        </div>
      </div>
    </div>
  );
};